class DependencyGraph {
    ...
    showPackageDetails(node) {
        ...
        const depsList = node.depends_on.length > 0
            ? node.depends_on.map(dep => `<li>${this.escapeHtml(dep)}</li>`).join('')
            : '<div class="empty-list">No dependencies</div>';

        const reqsList = node.required_by.length > 0
            ? node.required_by.map(req => `<li>${this.escapeHtml(req)}</li>`).join('')
        ...
        details.innerHTML = `
            ...
            <div class="detail-section">
                ...
                <span class="badge ${node.explicit ? 'explicit' : 'dependency'}">
                    ${node.explicit ? 'Explicitly Installed' : 'Dependency'}
                </span>
            </div>
            <div class="detail-section">
                <strong>Dependencies (${node.depends_on.length})</strong>
                ${node.depends_on.length > 0 ? '<ul>' + depsList + '</ul>' : depsList}
            </div>
            <div class="detail-section">
                <strong>Required By (${node.required_by.length})</strong>
                ${node.required_by.length > 0 ? '<ul>' + reqsList + '</ul>' : reqsList}
            </div>
        `;
    }
    ...
    updateStats() {
        ...
        document.getElementById('package-count').textContent =
            `${total} packages (${explicit} explicit, ${dependencies} dependencies)`;
    }
